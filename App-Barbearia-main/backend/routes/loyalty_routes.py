...
# Adicione no final do arquivo loyalty.py, antes do último router

@router.get("/client/email/{email}")
async def get_client_points_by_email(
    email: str,
    db: AsyncSession = Depends(get_db),
):
    ""Public: get client points by email""
    print(f"🔍 Buscando pontos para email: {email}")
    
    # Buscar cliente por email - você precisa adicionar o campo client_email na tabela
    # Por enquanto, vamos buscar por telefone se o email estiver salvo em outro lugar
    
    # Opção 1: Se a tabela LoyaltyPoints tem campo client_email
    try:
        result = await db.execute(
        ...
        # Se não tem campo email, buscar em appointments
        print("Campo client_email não existe, buscando em appointments...")
        # Buscar appointments com este email para encontrar o telefone
        appointment_result = await db.execute(
            select(Appointment).where(Appointment.client_email == email).limit(1)
        )
        appointment = appointment_result.scalar_one_or_none()
        
        if appointment and appointment.client_phone:
            # Encontrou o telefone pelo email nos appointments
            result = await db.execute(
                select(LoyaltyPoints).where(LoyaltyPoints.client_phone == appointment.client_phone)
            )
            client = result.scalar_one_or_none()
    
    if not client:
        print(f"❌ Cliente não encontrado para email: {email}")
    ...
    print(f"✅ Cliente encontrado: {client.client_phone} - {client.points} pontos")
    
    return {
        "client_phone": client.client_phone,
        "client_email": getattr(client, 'client_email', email),
        "client_name": client.client_name,
        "points": client.points,
        "total_earned": client.total_earned,
        "total_redeemed": client.total_redeemed,
...
@router.get("/client/email/{email}/history")
async def get_client_history_by_email(
    email: str,
    db: AsyncSession = Depends(get_db),
):
    ""Get client transaction history by email""
    print(f"📜 Buscando histórico para email: {email}")
    ...
    try:
        ...
        # Buscar appointments para encontrar telefone
        appointment_result = await db.execute(
            select(Appointment).where(Appointment.client_email == email).limit(1)
        )
        appointment = appointment_result.scalar_one_or_none()
    ...
    history_result = await db.execute(
        select(LoyaltyTransaction)
        .where(LoyaltyTransaction.client_phone == client.client_phone)
        .order_by(LoyaltyTransaction.created_at.desc())
        .limit(50)
    )
    txns = history_result.scalars().all()
    
    print(f"✅ Encontradas {len(txns)} transações")
