#!/usr/bin/env python3
"""
Final Comprehensive Backend Test Suite for Barbershop Manager API
Testing new public booking and barber schedule management endpoints - FULL FLOW
"""
...
TEST_BARBER_ID = "test_schedule_final"
TEST_BARBER_EMAIL = "schedule_final@test.com"
TEST_BARBER_NAME = "Final Test Barber"
...
class ComprehensiveBookingTester:
    ...
    def test_complete_schedule_workflow(self):
        ...
        print("\n📋 Step 2: Testing Complete Schedule Management Workflow...")
        
        # Test GET /api/schedule/ - should return empty list initially
        print("2.1. Testing GET /api/schedule/ (empty)")
        ...
        print("2.2. Testing POST /api/schedule/bulk (Mon-Fri)")
        bulk_data = {
        ...
        print("2.3. Testing GET /api/schedule/ (populated)")
        response = requests.get(f"{BACKEND_URL}/schedule/", headers=self.auth_headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}"
        schedules = response.json()
        ...
        if self.created_schedule_ids:
            ...
            print(f"2.4. Testing DELETE /api/schedule/{schedule_id_to_delete}")
        ...
        print("✅ Schedule management workflow completed successfully!")

    def test_complete_public_booking_workflow(self):
        """Test complete public booking workflow"""
        print("\n📋 Step 3: Testing Complete Public Booking Workflow...")
        
        # Test GET /api/public/services - should return services list
        print("3.1. Testing GET /api/public/services")
        response = requests.get(f"{BACKEND_URL}/public/services")
        assert response.status_code == 200, f"Expected 200, got {response.status_code}"
        services = response.json()
        assert isinstance(services, list), "Expected services list"
        
        # Find or create a service to use for booking
        if services:
        ...
        # Test GET /api/public/available-slots
        print(f"3.2. Testing GET /api/public/available-slots for {test_date} (Monday)")
        ...
        if not available_slots:
            print("⚠️  No available slots found - this may indicate schedule was cleared earlier")
            # Recreate a simple schedule for Monday
            print("Creating a fresh Monday schedule for testing...")
            simple_schedule = {
                "days": [0],  # Just Monday
                "start_time": "10:00",
                "end_time": "16:00", 
            ...
            response = requests.post(f"{BACKEND_URL}/schedule/bulk", 
                                   json=simple_schedule, 
                                   headers=self.auth_headers)
            assert response.status_code == 201, "Failed to create test schedule"
            ...
            slots_data = response.json()
            available_slots = slots_data['slots']
            print(f"✅ After creating schedule: Found {len(available_slots)} available slots")
        
        if not available_slots:
            print("❌ Still no available slots - cannot complete booking test")
            return False
        
        # Test POST /api/public/book - book an appointment
        print("3.3. Testing POST /api/public/book")
        ...
        booking_data = {
            ...
            "notes": "Test booking via public API"
        }
        ...
        print("3.4. Testing slot availability after booking")
        response = requests.get(f"{BACKEND_URL}/public/available-slots?date_str={test_date}")
        assert response.status_code == 200, f"Expected 200, got {response.status_code}"
        ...
        print(f"✅ Booked slot correctly removed from availability ({len(available_slots_after)} remaining)")
        
        # Test POST /api/public/book with same time - should get 409 conflict
        print("3.5. Testing duplicate booking prevention")
        response = requests.post(f"{BACKEND_URL}/public/book", json=booking_data)
        ...
        print("✅ Public booking workflow completed successfully!")
        return True

    def test_schedule_cleanup(self):
        """Test DELETE /api/schedule/ - clear all remaining"""
        print("\n📋 Step 4: Testing Schedule Cleanup...")
        print("4.1. Testing DELETE /api/schedule/ (clear all)")
    ...
    async def cleanup_test_data(self):
        ...
        try:
            ...
            async with async_session_factory() as db:
                ...
                await db.execute(
                    delete(Appointment).where(Appointment.client_name == 'Maria Santos')
                )
    ...
    async def run_all_tests(self):
        ...
        print("🚀 Starting Comprehensive Barber Booking API Test Suite")
        ...
        try:
            ...
            self.test_complete_schedule_workflow()
            
            # Step 3: Public Booking 
            booking_success = self.test_complete_public_booking_workflow()
            
            # Step 4: Schedule cleanup
            self.test_schedule_cleanup()
            
            if booking_success:
                print("\n🎉 ALL COMPREHENSIVE TESTS COMPLETED SUCCESSFULLY!")
                print("\n✅ VERIFICATION SUMMARY:")
                print("   • Schedule Management: ✅ CREATE, READ, UPDATE, DELETE all working")
                print("   • Public Services API: ✅ Returns service list correctly")
                print("   • Available Slots API: ✅ Calculates availability correctly")
                print("   • Public Booking API: ✅ Books appointments and prevents conflicts")
                print("   • Authentication: ✅ Protected endpoints require barber role")
                print("   • Database Integration: ✅ All CRUD operations working properly")
                return True
            else:
                print("\n⚠️  Tests completed with booking flow limitations")
                return False
...
async def main():
    """Main test runner"""
    tester = ComprehensiveBookingTester()
