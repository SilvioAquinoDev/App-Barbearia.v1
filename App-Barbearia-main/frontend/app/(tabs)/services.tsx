...
export default function Services() {
  ...
  const loadServices = async () => {
    try { 
      const r = await api.get('/services?active_only=false'); 
      // Ordenar serviços: ativos primeiro, depois inativos
      const sortedServices = r.data.sort((a: any, b: any) => {
  ...
  const handleToggleActive = async (service: any) => {
    // Adiciona o ID do serviço ao set de pendentes
    setPendingToggles(prev => new Set(prev).add(service.id));
    
    // Armazena o novo estado visual localmente
    const newToggleState = !service.is_active;
    setLocalToggleStates(prev => new Map(prev).set(service.id, newToggleState));
    
    // Aguarda 1 segundo antes de fazer a requisição e mudar a posição
    setTimeout(async () => {
      try { 
        await api.put(`/services/${service.id}/toggle-active`);
        // Recarrega os serviços do backend para garantir consistência
        await loadServices();
      } catch { 
        Alert.alert('Erro', 'Falha ao atualizar servico');
  ...
  // Função para obter o estado atual do switch (local ou do serviço)
  const getSwitchValue = (service: any) => {
    if (localToggleStates.has(service.id)) {
      return localToggleStates.get(service.id);
    }
...
const styles = StyleSheet.create({
  ...
  photosButton: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8, borderWidth: 1, marginTop: 8 },
